...
export default function SabbathCountdown() {
  ...
  if (!mounted) {
    return (
      <div className="bg-white dark:bg-dark-100 border border-gray-200 dark:border-dark-200 rounded-xl p-8">
        ...
        <div className="flex justify-center gap-3">
          {['Days', 'Hours', 'Minutes', 'Seconds'].map((label) => (
            <div key={label} className="text-center">
              <div className="bg-gray-900 dark:bg-dark-300 text-white text-3xl font-bold rounded-lg w-16 h-16 flex items-center justify-center">
                --
              </div>
              <span className="text-sm text-gray-500 dark:text-gray-400 mt-2 block">{label}</span>
            </div>
          ))}
        </div>
      </div>
